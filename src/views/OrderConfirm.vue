...
export default {
    ...
    computed:{
        orderTotal(){
            return this.itemSubTotal + this.shipping - this.discount + this.tax;
        },
        itemSubTotal(){
            var sum=0;
    ...
    methods:{
        ...
        payment(){
          ...
          axios.post('/users/payment',{addressId:addressId,orderTotal:this.orderTotal})
          .then((response)=>{
            var res=response.data
            if(res.status=='1'){
              alsert(res.msg)
              ...
              this.$router.push({path:'/orderSuccess?orderId='+result.orderId})
            }
          })
        }
    }
