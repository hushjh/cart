        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" @click="checkAddress(index,item.addressId)" :class="{'check':index==checkIndex}">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">{{item.streetName}}</dd>
                      <dd class="tel">{{item.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" @click="delConfirm(item.addressId)">
                      <a href="javascript:;" class="addr-del-btn">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefaultAddr(item.addressId)"><i>Set default</i></a>
                    </div>
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showMore" :class="{'open':limit==addressList.length}">
                  more
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{addressId:checkedAddressId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <nav-footer></nav-footer>
      <my-model :mdshow="addressDelConfirmMd" @close="closeMd">
        <p slot="message">你确定要删除此地址吗?</p>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="del">确定</a>
          <a href="javascript:;" class="btn btn--m" @click="addressDelConfirmMd=false">取消</a>
        </div>
      </my-model>
  </div>
</template>
...
export default {
  ...
  methods:{
      ...
      setDefaultAddr(addressId){
          axios.post('/users/address/setDefault',{addressId:addressId})
          .then((response)=>{
              var res=response.data;
              if(res.status=='0'){
                  this.init();
      ...
      delConfirm(addressId){
        this.addressDelConfirmMd=true;
        this.addressId=addressId;
      },
      del(){
        this.addressDelConfirmMd=false;
        axios.post('/users/address/del',{addressId:this.addressId})
        .then((response)=>{
          var res=response.data;
          if(res.status=='0'){
            this.init();
