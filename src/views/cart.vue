<template>
  <div>
              ...
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="mdDelConfirm(item)">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':btn_checkout}" @click="checkout">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <my-model :mdshow="mdCartDel" @close="closeMd">
      <p slot="message">你确定要删除此条数据吗</p>
      <div slot="btnGroup">
        <a class="btn btn--m" @click="cartDel">确定</a>
        <a class="btn btn--m" @click="mdCartDel=false">取消</a>
      </div>
    </my-model>
  </div>
</template>
<script>
    ...
    export default{
        ...
        computed:{
          ...
          checkAllFlag(){
            var isAll=true;
            isAll=!this.cartList.some((ele)=>{
              if(ele.checked =='0')return true;
            })
            return isAll;
          },
          btn_checkout(){
        ...
        methods:{
            ...
            cartDel(){
              this.mdCartDel=false;
              axios.post('/users/cart/del',{productId:this.delItem.productId})
              .then((response)=>{
                let res=response.data;
                if(res.status=='0'){
                  this.init();
            ...
            editCart(flag,item){
              ...
              axios.post('/users/cart/edit',{productId:item.productId,productNum:item.productNum,checked:item.checked})
              .then((response)=>{
                let res=response.data;
                if(res.status=='0'){
                  let num=0
            ...
            checkAll(){
              ...
              axios.post('users/cart/checkAll',{checkAllFlag:flag})
              .then((response)=>{
                let res=response.data;
                if(res.status=='0'){
                  this.init();
